...
class WechatAuth {
    ...
    public function newsAddMaterial(){
        $articles = func_get_args();
        $data['articles'] = $articles;
        echo "<pre>";
        return $this->api('material/add_news', $data);
    }
    /**
     * 获取媒体资源下载地址
     * 注意：视频资源不允许下载
     ...
     */
    public function messagePreviewSend($msgtype, $openid, $content){
        if($msgtype == "text"){
            $data = array(
                'touser'=>$openid,
            ...
            $data = array(
                'touser'=>$openid,
                $msgtype=>array("media_id"=>$content),
                'msgtype'=>$msgtype
                );
        }
...
//模板消息接口
    public function messageTemplateSend($openid, $template_id, $url, $name, $money){
        $data = array(
            'touser' => $openid,
            'template_id'=>$template_id,//Oh1wukwD5CXtNR7xafrWyNixgh7e225om5Y2goHdKkQ
            'url'=>$url,//http://www.qq.com
            'data' => array(
                'name' => array('value' => $name, 'color'=>'#173177'),
                'money' => array('value' => '本次金额'.$money, 'color'=>'#173177'), 
                'date' => array('value' => date('Y-m-d'), 'color'=>'#173177'), 
                ),
            );
    ...
    public function tagsUpdate($id, $name){
        ...
        return $this->api('tags/update', $data);
    }
    /**
     * 删除标签
     * @param  number $id   标签ID
    ...
    public function tagsDelete($id){
        ...
        return $this->api('tags/delete', $data);
    }
    /**
     * [获取标签下粉丝列表]
     ...
     * @return [type]              [粉丝列表]
     */
    public function getUsersByTag($tagid, $next_openid = null){
        $data = array('tagid' => $tagid, 'next_openid' => $next_openid);
        return $this->api('user/tag/get', $data);
    }
    /**
     * [batchTags 批量为用户打标签]
     * @param  [int] $tagid    [description]
     * @param  [array] $userlist [description]
     * @return [msg]           [description]
     */
    public function batchTags($tagid, $userlist){
        $data = array('tagid' => $tagid, 'openid_list' => $userlist);
        var_dump($data);
